...
// this page. Wrapped in try/catch since localStorage can throw in some
// contexts (private browsing, storage blocked) — falls back to blank.
const TEST_PHONE_STORAGE_KEY = "sehatpotli_admin_whatsapp_test_phone";

function readStoredTestPhone() {
  try {
    return localStorage.getItem(TEST_PHONE_STORAGE_KEY) || "";
  } catch {
    return "";
  }
}
...
const NotificationSettings = () => {
  ...
  const [testPhone, setTestPhone] = useState(readStoredTestPhone);
  ...
  const handleTestPhoneChange = (e) => {
    ...
    setTestPhone(value);
    try {
      localStorage.setItem(TEST_PHONE_STORAGE_KEY, value);
    } catch {
      // ignore — private browsing / storage blocked, just won't persist
    }
  };
